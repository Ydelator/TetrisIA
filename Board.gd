...
func _ready():
	...
	$GAME_HUD.get_node("Start").pressed.connect(new_game)
...
func new_game():
	...
	game_running = true
	steps = [0,0,0]
	$GAME_HUD.get_node("GameOver").hide()
	$GAME_HUD.get_node("ScoreLabel").text = "SCORE: 0"
	clear_board()
	clear_panel()
	clear_piece()
...
func check_rows():
	...
	while row > 0:
		...
		if count == COLS:
			shift_rows(row)
			score += REWARD
			$GAME_HUD.get_node("ScoreLabel").text = "SCORE: " + str(score)
			speed += ACCEL
		else:
			row -= 1
